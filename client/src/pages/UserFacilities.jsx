  ...
  useEffect(() => {
    ...
    const loadUserFacility = async () => {
      try {
        ...
        setFacility(data?.userFacilities?.[0] || null);
      } catch (error) {
        const message = error?.response?.data?.message || 'Failed to load user facilities';
        toast.error(message);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-cyan-950 flex items-center justify-center text-white">
        <p className="text-lg font-semibold tracking-wide">Loading user facilities...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-cyan-950 text-slate-100">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <article className="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl p-6 sm:p-8 shadow-2xl shadow-slate-950/30">
          <p className="text-xs uppercase tracking-[0.35em] text-cyan-200/80">User Facilities</p>
          <h1 className="mt-2 text-3xl sm:text-4xl font-semibold text-white">Selected facility</h1>
          <p className="mt-2 text-slate-300 max-w-2xl">Only the facility selected by the current user is shown here.</p>

          <div className="mt-6 rounded-2xl border border-white/10 bg-slate-950/40 px-5 py-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div>
                <p className="text-lg font-semibold text-white">{facility.name}</p>
                <p className="text-sm text-slate-400 mt-1">
                  {facility.date} at {facility.time}
                </p>
              </div>
            ...
            <p className="mt-4 text-sm text-slate-500">{facility.note}</p>
          </div>
        </article>
      </div>
    </div>
